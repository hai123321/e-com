...
import { loginUser, fetchMe, getGoogleLoginUrl } from '@/lib/auth'
...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetchMe(token).then(user => {
        if (user) {
          setUser(user, token)
          addToast(t.auth.greeting(user.name), 'success')
          router.replace('/')
        }
      })
    }
  }, [searchParams, setUser, addToast, router, t])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast(t.auth.greeting(result.user.name), 'success')
      router.push('/')
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 text-sm">{t.auth.loginWelcome}</p>
          </div>
          ...
          <div className="relative my-5">
            ...
            <div className="relative flex justify-center">
              <span className="px-3 bg-white text-gray-400 text-xs">{t.auth.or}</span>
            </div>
          </div>
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            {t.auth.noAccount}{' '}
            <Link href="/dang-ky" className="text-primary-700 font-semibold hover:underline">
              {t.auth.signUpLink}
            </Link>
          </p>
        </div>
